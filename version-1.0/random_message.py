...
def main(config=None):
    ...
    state_dim = env.observation_space[0].shape[0] + config["message_len"]
    ...
    for i_episode in range(1, max_episodes + 1):
        state = env.reset()
        state = [np.concatenate([s, np.random.uniform(low=-1, high=1, size=config["message_len"])]) for s in state]
